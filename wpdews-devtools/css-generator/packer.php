...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'health') {
		echo json_encode(array(
			'success' => true,
			'message' => 'Packer endpoint is available.'
		));
		exit;
	}
	...
	if ($action !== 'pack') {
		echo json_encode(array(
			'success' => false,
			'error' => 'Unsupported action. Available actions: "pack", "health".'
		));
		exit;
	}
	...
	$base62 = isset($_POST['base62']) && $_POST['base62'] === 'true';
	$fastDecode = isset($_POST['fastDecode']) ? $_POST['fastDecode'] === 'true' : true;
	$specialChars = isset($_POST['shrinkVars']) && $_POST['shrinkVars'] === 'true';
	$encoding = $base62 ? 62 : 0;

	try {
		$packer = new JavaScriptPacker($code, $encoding, $fastDecode, $specialChars);
		$packed = $packer->pack();
		...
		$ratio = 0;
		if ($originalSize > 0) {
			$ratio = round((1 - ($packedSize / $originalSize)) * 100, 1);
		}
